...
for _ in range(5):
    ...
    y_matrix.append(y_test)

y_matrix = list(zip(*y_matrix))
y_matrix = [[round(sum([y_list[0] for y_list in matrix])/len(matrix))] for matrix in y_matrix]
print(f'Predição: {y_matrix}')
